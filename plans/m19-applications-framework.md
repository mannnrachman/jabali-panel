...
- **Branch + commit, do not push.** Every step gets its own feature
  ...
  `root@10.0.3.13` via scp/rsync, never `git push`. The user merges to
...
- **Shipping cadence per step:** commit on a feature branch, build
  binaries, scp + rsync to `root@10.0.3.13`, restart, smoke-test. The
...
3. Existing WordPress installs still work end-to-end on
   `https://10.0.3.13` (smoke: list view loads, install modal opens,
...
go build ./... && make test
# After deploy:
ssh root@10.0.3.13 "mariadb jabali_panel -e 'SHOW CREATE TABLE \
  application_installs\\G' | grep -E 'app_type|UNIQUE KEY'"
ssh root@10.0.3.13 "mariadb jabali_panel -e 'SELECT * FROM \
  schema_migrations'"
