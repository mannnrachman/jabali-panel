...
func TestFilesStatHandler(t *testing.T) {
	testUser := currentTestUser(t)

	tests := []struct {
		name      string
		input     filesStatParams
		wantError bool
		wantCode  string
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := filesStatHandler(context.Background(), params)

			if (err != nil) != tt.wantError {
				t.Errorf("filesStatHandler: expected error = %v, got %v", tt.wantError, err)
			}
