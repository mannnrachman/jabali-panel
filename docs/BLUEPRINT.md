...
| M33: Malware detection stack (Waves A+B of 5) | 2026-04-27 (partial) | ADR-0067 (DRAFT); blueprint at `plans/m33-malware-detection.md`; branch `m33/malware-detection` (`63b5dc3` → `566646e`). Waves A+B shipped: schema (migration 000076 — malware_quarantine + malware_events + yara_custom_rules + tetragon_policy_state + malware_settings singleton), 5 GORM repos, install.sh `install_malware_stack` provisioning ClamAV + LMD 1.6.6 (sha256-pinned tarball) + YARA + sysctl bump (`fs.inotify.max_user_watches=524288`), 10 agent commands `security.malware.{status,scan_path,scan_user,scan_status,quarantine.list/restore/delete,update_signatures,monitor.reload,event}`, 11 panel-api routes incl. RequireLocalhost-gated `/event` ingest, new admin tab between CrowdSec and UFW with 5 sub-tab cards (Overview/Quarantine/Events/Manual scan/Settings). **Waves C-E deferred to next session:** sessionwatcher fsnotify ingestion (Step 6), inotify monitor systemd unit + reconciler hook + M14 wiring with 2 event sources + 12 templates (Step 7), YARA admin upload UI (Step 8), Tetragon install + tetragon-relay + policy UI (Step 9), retention timers + E2E + runbook + ADR-0067 finalize (Step 10). Tab is live but events table stays empty until Step 6+7 land. |
