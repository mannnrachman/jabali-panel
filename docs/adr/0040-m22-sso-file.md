...
3. **PHP template (file content).** The file is self-contained — only `wp-load.php` is included. The execution sequence (top to bottom):
   1. **Prefetch / non-GET guard**: short-circuit on `Sec-Purpose: prefetch`, `Purpose: prefetch`, `X-Moz: prefetch`, `X-Purpose: preview`, or any non-`GET` method → return 204 without entering single-use state.
