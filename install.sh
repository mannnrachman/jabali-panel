...
main() {
  ...
  install_jabali_slices
  install_kratos
  # M6 mail stack: Stalwart (SMTP/IMAP/JMAP) + Bulwark (Next.js webmail).
  # Both services install disabled — they enable on first panel
  # domain.email_enable call. Safe to run on hosts that never enable
  # email: dormant disk + memory cost only.
  install_stalwart
  install_bulwark
  install_php_pool_template
  build_frontend
