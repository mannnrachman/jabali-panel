...
func TestFilesReadHandler(t *testing.T) {
	testUser := currentTestUser(t)

	tests := []struct {
		name      string
		input     filesReadParams
		wantError bool
		wantCode  string
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := filesReadHandler(context.Background(), params)

			if (err != nil) != tt.wantError {
				t.Errorf("filesReadHandler: expected error = %v, got %v", tt.wantError, err)
			}
