...
- On the smoke-test host (a disposable LXC or VM — **not** `10.0.3.13` except during step 6 cutover): a user's vhost serves PHP via the per-user slice and `systemd-cgls /jabali.slice` shows workers + shell under the user's own child slice.
...
- **Do NOT** skip the smoke host. Step 6 cutover directly on `10.0.3.13` without a disposable host first is reckless.
