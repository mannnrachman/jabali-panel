...
Agent runs as root. For every write op (upload, mkdir, rename), agent performs the op then `chown <user>:<user>`, `chmod 0644` (files) / `0755` (dirs). Sensitive files (e.g. `wp-config.php`-shaped) get `0640` — future convention, enforced by caller not filesafe. No `setresuid`, no per-user helper daemon. This matches M9 per-user PHP-FPM isolation (each pool runs as `<user>:<user>`) and M10 WordPress install (`wordpress_install.go:285`, `wordpress_clone.go:371`). Do NOT put files in the `www-data` group — that would leak read access to the nginx worker and weaken per-user isolation. nginx reads static files via world-read bit on 0644; PHP-FPM reads user files via user UID match. Scope-clamp is enforced in the shared validator, called in BOTH panel-api AND panel-agent (defense in depth — same pattern as M8 cron).
...
3. Ownership is `<user>:<user>`, mode `0644` files / `0755` dirs (never root:root, never `www-data` group). Matches M9 per-user FPM isolation + M10 WordPress pattern.
