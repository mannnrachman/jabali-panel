...
func TestFilesListHandler(t *testing.T) {
	testUser := currentTestUser(t)

	tests := []struct {
		name      string
		input     filesListParams
		wantError bool
		wantCode  string
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := filesListHandler(context.Background(), params)

			if (err != nil) != tt.wantError {
				t.Errorf("filesListHandler: expected error = %v, got %v", tt.wantError, err)
			}
